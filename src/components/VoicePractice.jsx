...
// Función para medir la similitud entre cadenas
const similarity = (s1, s2) => {
  const dist = levenshtein(s1.toLowerCase(), s2.toLowerCase());
  const maxLen = Math.max(s1.length, s2.length);
  return 1 - dist / maxLen; // Puntuación de similitud (de 0 a 1)
};
...
function VoicePractice() {
  ...
  const handleRecognition = () => {
    startRecognition((result) => {
      ...
      // Usa la función de similitud (Levenshtein) para permitir flexibilidad
      const similarityScore = similarity(normalizedResult, normalizedPhrase);
