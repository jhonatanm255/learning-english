...
// // Al instalar el service worker, cacheamos los archivos y notificamos la nueva versión
// self.addEventListener("install", (event) => {
//   console.log("Service Worker: Install event triggered");
//   const newVersion = "v2"; // Define manualmente la versión nueva
//   event.waitUntil(
...
// // Recibir mensajes desde la app (como SKIP_WAITING para aplicar actualización)
// self.addEventListener("message", (event) => {
//   if (event.data && event.data.type === "SKIP_WAITING") {
//     console.log("Service Worker: SKIP_WAITING received, skipping waiting");
//     self.skipWaiting(); // Ejecutar solo si el cliente lo solicita
...
// // Al activar el service worker, limpiar las versiones anteriores del cache
// self.addEventListener("activate", (event) => {
//   console.log("Service Worker: Activate event triggered");
//   // Borrar cachés antiguos si la versión del cache ha cambiado
//   event.waitUntil(
...
// // Manejo de las solicitudes de la red y el cache
// self.addEventListener("fetch", (event) => {
//   console.log(`Service Worker: Fetch event for ${event.request.url}`);
//   event.respondWith(
//     caches.match(event.request).then((cachedResponse) => {
